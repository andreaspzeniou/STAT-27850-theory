---
title: "E-Values Grouping Deltas"
...
```{r}
# Comparison of Base e-BH vs. Grouping e-BH - Graphing FDR and Power for Various Deltas for both methods
...
for (del in delta_trials) {
  ...
  for (k in 1:trials) {
    ...
    storey_indices <- e_bh_storey(sorted_e_values, alpha_storey/ pi_0_hat)
    two_step_storey_indices <- e_bh_storey(sorted_e_values,(alpha_storey - beta) / pi_0_hat2)
...
ggplot(data_long, aes(x = delta_trials, y = Power, color = Group)) +
  ...
  scale_color_discrete(labels = c(
    "avg_power_nongroup" = "Label for Non-Group",
    "avg_power_storey" = "Label for Storey",
    "avg_power_twostp_storey" = "Label for Two-Step Storey"
  ))

```

```{r}
# Create a data frame with your vectors
data <- data.frame(
  delta_trials = delta_trials,
